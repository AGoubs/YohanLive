              ...
              <tr class="px-3" wire:key="{{ $event->id }}" style="cursor:pointer">
                ...
                @if (auth()->user()->isAdmin())
                  <td class="text-md-center action-col">
                    ...
                    <span>
                      <i class="cursor-pointer fas fa-trash text-secondary" data-bs-toggle="tooltip" data-bs-original-title="Supprimer l'évènement" onclick="deleteEvent({{ $event->id }},'{{ $event->Nom }}')"></i>
                    </span>
                  </td>
                @endif
              </tr>
...
<script>
  ...
  function deleteEvent(id, name) {
    if (confirm("Supprimer cet évènement : " + name + " ?")) {
      @this.deleteEvent(id);
    }
  }
</script>
